...
export default function AspireEventJune2026() {
  useSEO({
    ...
    canonical: 'https://blacktechstreet.ai/aspire/events/june-2026',
  });
  ...
  const juneEvent = useMemo(() => {
    return aspireEvents2026.find(e => e.id === 'june-2026')!;
  }, []);

  const handleRegister = () => {
    setSelectedEvent(juneEvent);
    setIsModalOpen(true);
  };
                ...
                <div className="grid md:grid-cols-2 gap-8 items-center">
                  ...
                  <motion.div className="space-y-4" variants={containerVariants}>
                    <motion.h3 variants={itemVariants} className="text-3xl md:text-4xl font-display font-bold">
                      {format(juneEvent.date, 'MMMM d, yyyy')}
                    </motion.h3>
                    
                    <motion.div variants={itemVariants} className="flex flex-col gap-3">
                      {[
                        { icon: Calendar, text: format(juneEvent.date, 'EEEE') },
                        { icon: Clock, text: juneEvent.time },
                      ].map((item, idx) => (
                        <motion.div 
                          key={idx}
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="text-lg md:text-xl font-medium text-foreground group-hover:text-primary transition-colors">{juneEvent.locationFull}</span>
                        </motion.div>
                  ...
                  <motion.div variants={itemVariants} className="flex justify-center md:justify-end">
                    <FlipClock targetDate={juneEvent.date} />
                  </motion.div>
                </div>
              </div>
            </motion.div>
